package com.localdrive.plugin.backgroundvideo;
...
public class VideoOverlay extends ViewGroup implements TextureView.SurfaceTextureListener {
    ...
    public void Start(String filePath) throws Exception {
        ...
        try {
            ...
            CamcorderProfile profile = getProfile(cameraParameters);
            ...
            if (mRecordAudio) {
                ...
                // Without audio
                //Log.w(TAG, "jrAraujo: "+(Integer.toString(profile.OutputFormat)));
                mRecorder.setOutputFormat(MediaRecorder.OutputFormat.MPEG_4);
                mRecorder.setVideoFrameRate(profile.videoFrameRate);
                mRecorder.setVideoSize(profile.videoFrameWidth, profile.videoFrameHeight);
                mRecorder.setVideoEncodingBitRate(profile.videoBitRate);
    ...
    public CamcorderProfile getProfile(Camera.Parameters cameraParameters) {
        ...
        if (mQuality.equals("high")) {
            profile.videoBitRate = 4000000;
        } else if (mQuality.equals("medium")) {
            profile.videoBitRate = 1500000;
        } else if (mQuality.equals("low")) {
            profile.videoBitRate = 512000;
        }
