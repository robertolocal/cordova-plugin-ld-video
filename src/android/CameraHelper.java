...
class CameraHelper {
    ...
    static Camera.Size getResolution(Camera.Parameters cp, String quality) {
        ...
        Camera.Size small = sl.get(0);
        Camera.Size bigger = sl.get(0);

        for (Camera.Size s : sl) {
            if ((s.height * s.width) < (small.height * small.width))
                small = s;

            if ((s.height * s.width) > (bigger.height * bigger.width))
                bigger = s;
        }

        return quality.equals("low") ? small : bigger;
    }
